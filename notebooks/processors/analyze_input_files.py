...
# DBTITLE 1,Define UDF for Relative Path Calculation
def calculate_relative_path(file_path: str, base_dir: str) -> str:
    """Calculate relative path from base directory."""
    return os.path.relpath(file_path, base_dir) if file_path and base_dir else None


calculate_relative_path_udf = udf(lambda path: calculate_relative_path(path, input_dir), StringType())
...
schema = StructType(
    [
        ...
        StructField("input_file_relative_path", StringType(), True),
...
result_df = (
    ...
    .withColumn("input_file_relative_path", calculate_relative_path_udf(col("input_file_path")))
    .withColumn(
